package com.dyman.show3dmodel.view;

import android.content.Context;
import android.view.MotionEvent;

import com.dyman.show3dmodel.bean.ModelObject;


/**
 * Created by dyman on 16/7/25.
 */
public class PrintModelView extends  ModelView{

    private static final String TAG = "PrintModelView";

    public PrintModelView(Context context, ModelObject modelObject){
        super(context, modelObject);
    }

    /**
     *  触摸事件回调方法，支持动作：单指旋转
     * @param e
     * @return
     */
    @Override
    public boolean onTouchEvent(MotionEvent e) {
        float y = e.getY();
        float x = e.getX();

        switch (e.getAction() & MotionEvent.ACTION_MASK) {

            case MotionEvent.ACTION_DOWN:
                if (touchMode ==TOUCH_NONE && e.getPointerCount() == 1){
                    touchMode = TOUCH_DRAG;
                    mPreviousX = e.getX();
                    mPreviousY = e.getY();
                }
                break;

            case MotionEvent.ACTION_MOVE:
                if(touchMode == TOUCH_DRAG){
                    float dy = y - mPreviousY;//计算触控笔Y位移
                    float dx = x - mPreviousX;//计算触控笔X位移
                    mRenderer.yAngle += dx * TOUCH_SCALE_FACTOR;//设置沿x轴旋转角度
                    mRenderer.zAngle+= dy * TOUCH_SCALE_FACTOR;//设置沿z轴旋转角度
                }
                requestRender();
                mPreviousY = y;//记录触控笔位置
                mPreviousX = x;//记录触控笔位置
                break;

            case MotionEvent.ACTION_UP:
                if (touchMode == TOUCH_DRAG){ touchMode = TOUCH_NONE; }
                break;
        }

        return true;
    }


    /**
     *  更新打印进度
     * @param process
     */
    public void updatePrintProgress(float process) {
        printProgress = process;
    }
}
